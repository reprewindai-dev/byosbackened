...
async def get_cost_breakdown(
    ...
    """Get cost breakdown."""
    allocations = db.query(CostAllocation).filter(CostAllocation.workspace_id == workspace_id).all()
    ...
    for a in allocations:
        if group_by == "project":
            ...
            audit_log = db.query(AIAuditLog).filter(AIAuditLog.id == a.operation_id).first()
