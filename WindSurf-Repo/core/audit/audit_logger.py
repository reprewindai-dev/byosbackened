...
class AuditLogger:
    ...
    def append_event(self, *args, **kwargs):
        """Append a generic audit event."""
        # Temporarily disabled to isolate dict parameter issue
        pass
    
    def _map_action_to_event_type(self, action: str) -> str:
        """Map action string to audit event type."""
        from db.models.audit_event import AuditEventType
        ...
        elif 'enable' in action_lower and 'app' in action_lower:
            return AuditEventType.APP_ENABLED
        elif 'disable' in action_lower and 'app' in action_lower:
            return AuditEventType.APP_DISABLED
        elif 'create' in action_lower and 'workspace' in action_lower:
            return AuditEventType.WORKSPACE_CREATED
    ...
    def _determine_severity(self, action: str, success: bool) -> str:
        """Determine event severity based on action and outcome."""
        from db.models.audit_event import AuditEventSeverity
        
        if not success:
            return AuditEventSeverity.HIGH
        
        action_lower = action.lower()
        if any(keyword in action_lower for keyword in ['delete', 'remove', 'disable']):
            return AuditEventSeverity.MEDIUM
        elif any(keyword in action_lower for keyword in ['login', 'create', 'enable']):
...
def get_audit_event_logger() -> AuditLogger:
    """Factory to get the global audit logger instance."""
    return _audit_logger


def append_event(*args, **kwargs):
    """Append a generic audit event (placeholder for missing AuditEvent model)."""
    return _audit_logger.append_event(*args, **kwargs)
