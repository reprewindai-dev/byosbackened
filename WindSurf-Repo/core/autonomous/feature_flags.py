...
class FeatureFlags:
    ...
    def __init__(self):
        ...
        self._flags = {
            "autonomous_routing_enabled": os.getenv("AUTONOMOUS_ROUTING_ENABLED", "false").lower()
            == "true",
            "ml_cost_prediction_enabled": os.getenv("ML_COST_PREDICTION_ENABLED", "false").lower()
            == "true",
            "ml_routing_optimizer_enabled": os.getenv(
                "ML_ROUTING_OPTIMIZER_ENABLED", "false"
            ).lower()
            == "true",
            "model_retraining_enabled": os.getenv("MODEL_RETRAINING_ENABLED", "false").lower()
            == "true",
            "canary_deployment_enabled": os.getenv("CANARY_DEPLOYMENT_ENABLED", "false").lower()
            == "true",
            "auto_remediation_enabled": os.getenv("AUTO_REMEDIATION_ENABLED", "false").lower()
            == "true",
            "edge_routing_enabled": os.getenv("EDGE_ROUTING_ENABLED", "false").lower() == "true",
            "traffic_prediction_enabled": os.getenv("TRAFFIC_PREDICTION_ENABLED", "false").lower()
            == "true",
        }
