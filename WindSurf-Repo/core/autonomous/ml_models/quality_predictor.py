...
class QualityPredictorML:
    ...
    def predict_quality(
        ...
        # Check kill switch
        if not (
            feature_flags.is_enabled("ml_quality_prediction_enabled")
            or feature_flags.is_enabled("ml_cost_prediction_enabled")
        ):
        ...
        try:
            ...
            logger.warning("NumPy unavailable/blocked; using fallback quality prediction")
            return self._fallback_prediction(provider, operation_type)
    ...
    def suggest_provider_for_quality(
        ...
        if not best_provider:
            ...
            best_provider = max(
                predictions.keys(), key=lambda p: predictions[p]["predicted_quality"]
            )
    ...
    def train_model(
        ...
        new_model = MLModel(
            ...
            accuracy=Decimal(
                str(1.0 - mae)
            ),  # Accuracy = 1 - MAE (for quality, lower MAE = higher accuracy)
        ...
        if existing_production:
            old_mae = (
                1.0 - float(existing_production.accuracy) if existing_production.accuracy else 0.15
            )
