...
class RoutingOptimizerML:
    ...
    def select_provider(
        ...
        try:
            ...
            if not self._meets_constraints(selected, validated_constraints):
                ...
                fallback = available_providers[0] if available_providers else "openai"
                logger.warning(
    ...
    def update_routing_outcome(
        ...
        success = (
            actual_quality >= 0.7  # Minimum quality threshold
            and actual_latency_ms < 10000  # Maximum latency threshold
        )
    ...
    def learn_from_history(
        ...
        logger.info(f"Learned from {len(decisions)} historical decisions for {bandit_key}")
