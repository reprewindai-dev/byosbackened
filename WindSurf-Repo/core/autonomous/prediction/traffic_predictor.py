...
class TrafficPredictor:
    ...
    def _train_from_history(self, workspace_id: str):
        ...
        try:
            ...
            try:
                ...
                logger.warning("scikit-learn unavailable/blocked; skipping traffic model training")
                return
    ...
    def predict_spike(
        ...
        spikes = [p for p in predictions if p["predicted_requests"] > avg_requests * 2]
        ...
        if spikes:
            ...
            return {
                ...
                "spike_multiplier": (
                    spike["predicted_requests"] / avg_requests if avg_requests > 0 else 1.0
                ),
            }
