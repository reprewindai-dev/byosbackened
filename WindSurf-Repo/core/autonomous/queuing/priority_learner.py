    ...
    def update_priority_signal(
        ...
        if key not in self.learned_priorities:
            ...
            learned["avg_completion_time"] = (
                alpha * completion_time_seconds + (1 - alpha) * learned["avg_completion_time"]
            )

            # Update urgency ratio
            if accessed_immediately:
                learned["urgency_ratio"] = alpha * 1.0 + (1 - alpha) * learned["urgency_ratio"]
            else:
                learned["urgency_ratio"] = alpha * 0.0 + (1 - alpha) * learned["urgency_ratio"]
