...
class AnomalyMetadataSchema(BaseModel):
    ...
    affected_operations: Optional[List[str]] = Field(
        None, description="List of affected operation IDs"
    )
    ...
    def validate_jsonb(cls, data: Optional[Dict]) -> Optional[Dict[str, Any]]:
        ...
        if "operation_type" in data:
            if not isinstance(data["operation_type"], str):
                raise ValueError(
                    f"operation_type must be string, got: {type(data['operation_type'])}"
                )
        ...
        if "affected_operations" in data:
            if not isinstance(data["affected_operations"], list):
                raise ValueError(
                    f"affected_operations must be list, got: {type(data['affected_operations'])}"
                )
        ...
        if "related_anomaly_ids" in data:
            if not isinstance(data["related_anomaly_ids"], list):
                raise ValueError(
                    f"related_anomaly_ids must be list, got: {type(data['related_anomaly_ids'])}"
                )
