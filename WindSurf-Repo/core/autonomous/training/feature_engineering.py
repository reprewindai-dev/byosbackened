...
class FeatureEngineer:
    ...
    def extract_cost_features(
        ...
        features.append(
            float(estimated_output_tokens / input_tokens) if input_tokens > 0 else 0.0
        )  # Output/input ratio
    ...
    def extract_routing_features(
        ...
        for provider in available_providers:
            ...
            # Constraints
            features.append(
                float(constraints.get("max_cost", 0.01)) if constraints.get("max_cost") else 0.01
            )
            features.append(
                float(constraints.get("min_quality", 0.0))
                if constraints.get("min_quality")
                else 0.0
            )
            features.append(
                float(constraints.get("max_latency_ms", 10000))
            ...
            if db:
                ...
                features.extend(
                    [0.0, 0.0, 0.0, 0.0]
                )  # avg_cost, avg_quality, avg_latency, success_rate
    ...
    def _extract_historical_features(
        ...
        predictions = (
            ...
            .order_by(CostPrediction.created_at.asc())
            .limit(10000)
            .all()
    ...
    def _extract_provider_historical_features(
        ...
        qualities = [float(d.actual_quality) for d in decisions if d.actual_quality is not None]
        latencies = [d.actual_latency_ms for d in decisions if d.actual_latency_ms is not None]
        successes = [1.0 if d.actual_cost and d.actual_cost > 0 else 0.0 for d in decisions]
