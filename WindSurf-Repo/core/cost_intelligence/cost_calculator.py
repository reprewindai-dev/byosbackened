...
try:
    ...
    logger.warning(f"Failed to initialize ML cost predictor: {e}")
    cost_predictor_ml = None
...
class CostCalculator:
    ...
    def predict_cost(
        ...
        """
        # Try ML prediction first if workspace_id provided (ML currently disabled due to initialization issues)
        if False and use_ml and workspace_id and input_tokens and cost_predictor_ml:
            try:
        ...
        # Calculate cost
        if provider == "openai":
            input_cost = (Decimal(input_tokens) / Decimal(1_000_000)) * pricing.get(
                "input", Decimal("0")
            )
            output_cost = (Decimal(estimated_output_tokens) / Decimal(1_000_000)) * pricing.get(
                "output", Decimal("0")
            )
        ...
        # Find alternative providers (only if not in recursive call)
        alternatives = []
        if self._recursion_depth < self._max_recursion_depth:
            ...
            for alt_provider in ["huggingface", "openai", "local"]:
                ...
                alt_cost = self.predict_cost(
                    ...
                    model=model,
                    use_ml=False,  # Don't use ML for alternatives to avoid recursion
                ).predicted_cost
                savings = (
                    ((predicted_cost - alt_cost) / predicted_cost * 100) if predicted_cost > 0 else 0
                )
                alternatives.append(
                    {
    ...
    def record_actual_cost(
        ...
        error_percent = (
            abs((actual_cost - predicted_cost) / predicted_cost * 100) if predicted_cost > 0 else 0
        )
