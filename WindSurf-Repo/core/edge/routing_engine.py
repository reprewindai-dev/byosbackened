        ...
        with tracer.span(
                ...
                if data_residency.lower() in region_map:
                    ...
                    result = {
                        ...
                        "factors": {
                            "data_residency": True,
                            "latency_ms": (
                                latency_monitor.get_latency_stats(selected)["avg"]
                                if latency_monitor.get_latency_stats(selected)
                                else 0.0
                            ),
                        },
                    }
                    span.add_event(
                        "region_selected", {"region": selected, "reason": "data_residency"}
                    )
                    return result
            ...
            with tracer.span(
                ...
                if learned_region:
                    ...
                    span.add_event(
                        "region_selected", {"region": learned_region, "reason": "learned"}
                    )
                    return result
            ...
            if prioritize_latency:
                ...
                with tracer.span(
                    ...
                    result = {
                        "region": selected,
                        "reasoning": (
                            f"Lowest latency region (avg: {stats['avg']:.1f}ms)"
                            if stats
                            else "Lowest latency region"
                        ),
                        "factors": {
                ...
                with tracer.span(
                    ...
                    for region in region_manager.get_regions():
                        ...
                        latency_score = (
                            stats["avg"] / 1000.0 if stats and stats["avg"] > 0 else 1.0
                        )  # Normalize to 0-1
                        cost_score = (
                            float(costs["total_cost"]) / 0.01
                        )  # Normalize to 0-1 (assuming max $0.01)
