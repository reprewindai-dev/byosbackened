...
def validate_purpose_limitation(
    data: Dict[str, Any], stated_purpose: str, purpose_mapping: Dict[str, List[str]]
) -> bool:
    """
    ...
    if not data_fields.issubset(allowed_fields):
        ...
        logger.warning(
            f"Purpose limitation violation: fields {disallowed} not allowed for purpose {stated_purpose}"
        )
        return False
