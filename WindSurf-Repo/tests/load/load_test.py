...
pytestmark = pytest.mark.skip(reason="load test (requires running server)")
...
async def run_endpoint_test(
    ...
    async with httpx.AsyncClient(base_url=BASE_URL) as client:
        ...
        tasks = [client_worker(client) for _ in range(CONCURRENT_REQUESTS)]
...
async def run_load_tests():
    ...
    for endpoint, method, json_data in endpoints:
        try:
            stats = await run_endpoint_test(endpoint, method, json_data, headers)
