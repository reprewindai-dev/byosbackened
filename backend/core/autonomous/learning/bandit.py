...
class MultiArmedBandit:
    ...
    def select_arm(
        ...
        if np.random.random() < effective_exploration_rate:
            ...
            selected = np.random.choice(safe_providers)
            ...
            for provider in safe_providers:
                ...
                if arm["count"] < self.min_samples:
                    ...
                    ucb = np.sqrt(2 * np.log(total_count + 1) / arm["count"])
        ...
        for provider in providers:
            ...
            if max_latency_ms and "avg_latency_ms" in arm:
                if arm["avg_latency_ms"] > max_latency_ms:
                    logger.debug(f"Provider {provider} excluded: latency {arm['avg_latency_ms']} > {max_latency_ms}")
                    continue
            
            # Check cost constraint (if we have historical data)
            if max_cost and "avg_cost" in arm:
                if arm["avg_cost"] > float(max_cost):
                    logger.debug(f"Provider {provider} excluded: cost {arm['avg_cost']} > {max_cost}")
                    continue
            ...
            if failure_count >= 5:  # Circuit breaker threshold
                logger.debug(f"Provider {provider} excluded: circuit breaker open (failures: {failure_count})")
                continue
