    ...
    def predict_cost(
        ...
        if db:
            ...
            if canary_model and random.random() < 0.1:
                ...
                if canary_key not in self.models:
                    if self._load_model_from_s3(canary_key, s3_key=f"ml_models/{canary_model.s3_key}"):
                        logger.debug(f"Loaded canary model {canary_model.model_version} for workspace {workspace_id}")
                    else:
        ...
        if use_canary:
            ...
            if not model_obj:
                ...
                span.set_attribute("prediction_method", "fallback")
                span.add_event("fallback_prediction", {"reason": "model_load_failed"})
                return self._fallback_prediction(
                    provider, operation_type, input_tokens, estimated_output_tokens, model
                )
            ...
            # Prepare features
            with tracer.span(
                name="ml.feature_extraction",
                trace_id=trace_id,
                parent_span_id=span.span_id,
            ) as feature_span:
                features = self._extract_features(
                    operation_type, provider, input_tokens, estimated_output_tokens,
                    model, time_of_day, workspace_id
                )
                feature_span.set_attribute("feature_count", len(features))
            
            # Predict
            with tracer.span(
                name="ml.model_prediction",
                trace_id=trace_id,
                parent_span_id=span.span_id,
                attributes={
                ...
                prediction_span.set_attribute("predicted_cost", float(predicted_cost))
            ...
            span.set_attribute("prediction_method", "ml" if not is_canary else "ml_canary")
            span.set_attribute("predicted_cost", float(predicted_cost))
