...
class DriftDetector:
    ...
    def detect_drift(
        ...
        try:
            ...
            return {
                ...
                "recommendation": (
                    "Retrain model" if drift_detected else "No action needed"
                ),
                "severity": max(
                    data_drift.get("severity", "low"),
                    concept_drift.get("severity", "low"),
                    key=lambda s: ["low", "medium", "high", "critical"].index(s)
                ) if drift_detected else "low",
            }
        finally:
            if should_close:
                db.close()
    ...
    def _detect_data_drift(
        ...
        if model_type == "cost_predictor":
            ...
            training_predictions = db.query(CostPrediction).filter(
            ...
            recent_predictions = db.query(CostPrediction).filter(
                CostPrediction.workspace_id == workspace_id,
            ...
            drift_detected = len(significant_drifts) > len(drift_scores) * 0.2  # >20% of features drifted
        ...
        if model_type == "cost_predictor":
            ...
            for pred in recent_predictions:
                if pred.actual_cost and pred.predicted_cost:
                    error = abs(float(pred.actual_cost) - float(pred.predicted_cost)) / float(pred.actual_cost)
                    errors.append(error)
            ...
            recent_logs = db.query(AIAuditLog).filter(
