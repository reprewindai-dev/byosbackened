...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE anomalytype AS ENUM ('cost_spike', 'traffic_spike', 'quality_degradation', 'latency_spike', 'failure_rate_increase', 'unusual_pattern')")
    op.execute("CREATE TYPE anomalystatus AS ENUM ('detected', 'investigating', 'remediated', 'false_positive', 'resolved')")
    ...
    op.create_index('ix_routing_strategies_operation_type', 'routing_strategies', ['operation_type'])
    op.create_index('idx_routing_strategies_workspace_op', 'routing_strategies', ['workspace_id', 'operation_type'])
    ...
    op.create_index('idx_traffic_patterns_workspace_op', 'traffic_patterns', ['workspace_id', 'operation_type'])
    ...
    op.create_table(
        ...
        sa.Column('anomaly_type', postgresql.ENUM('cost_spike', 'traffic_spike', 'quality_degradation', 'latency_spike', 'failure_rate_increase', 'unusual_pattern', name='anomalytype'), nullable=False),
        sa.Column('severity', postgresql.ENUM('low', 'medium', 'high', 'critical', name='anomalyseverity'), nullable=False),
        sa.Column('status', postgresql.ENUM('detected', 'investigating', 'remediated', 'false_positive', 'resolved', name='anomalystatus'), nullable=False),
    ...
    op.create_index('idx_anomalies_workspace_detected', 'anomalies', ['workspace_id', 'detected_at'])
    ...
    op.create_index('idx_savings_reports_workspace_period', 'savings_reports', ['workspace_id', 'period_start', 'period_end'])
