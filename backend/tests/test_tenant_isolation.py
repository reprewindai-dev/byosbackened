...
def token_a(workspace_a: Workspace, user_a: User):
    ...
    return create_access_token(
        data={"sub": user_a.email, "workspace_id": workspace_a.id}
    )
...
def token_b(workspace_b: Workspace, user_b: User):
    ...
    return create_access_token(
        data={"sub": user_b.email, "workspace_id": workspace_b.id}
    )
...
class TestTenantIsolation:
    ...
    def test_direct_database_query_isolation(
        ...
        strategies_b = db.query(RoutingStrategy).filter(
            RoutingStrategy.workspace_id == workspace_b.id
        ).all()
        ...
        strategies_a = db.query(RoutingStrategy).filter(
            RoutingStrategy.workspace_id == routing_strategy_a.workspace_id
        ).all()
