...
const UACP_EVENT_PRICES = [
  ["UACP plan compile", "$1.50 Founding / $2.00 Standard / $2.50 Regulated"],
  ["UACP run execution", "$3 Founding / $4 Standard / $5 Regulated"],
      ...
      <header className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <div>
          ...
          <h1 className="font-display mt-2 text-[34px] font-semibold tracking-tight text-bone">
            UACP black-box runtime
          </h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-bone-2">
            This page wraps the actual UACPGemini control-plane app. The workspace shell handles tenant scope,
            activation state, and reserve posture; the embedded runtime remains the real black-box interface.
          </p>
        ...
        <aside className="frame p-4">
          ...
          <div className="mt-3 space-y-3 text-sm">
            {UACP_EVENT_PRICES.map(([label, price]) => (
              <div key={label} className="flex items-start justify-between gap-3 border-b border-rule/70 pb-2 last:border-0 last:pb-0">
      ...
      <section className="frame overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-rule/80 bg-ink-1/60 px-4 py-3">
          <div>
            ...
            <h2 className="font-display mt-1 text-base font-semibold text-bone">UACPGemini execution theater</h2>
          </div>
        ...
        <div className="relative bg-black">
            ...
            <div className="absolute inset-0 z-10 grid place-items-center bg-black/70 p-6 backdrop-blur-sm">
              <div className="max-w-lg rounded-xl border border-brass/30 bg-ink/95 p-5 text-center shadow-2xl">
                ...
                <p className="mt-3 text-sm leading-6 text-bone-2">
                  The UACP runtime is loaded behind the paid gate. Free Evaluation can browse the hub, but UACP
          ...
          <iframe
            title="UACPGemini wrapped runtime"
